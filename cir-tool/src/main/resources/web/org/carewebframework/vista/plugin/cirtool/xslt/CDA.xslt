...
<!--
  ...
  Revision History:  04/26/13 Dave Blaeser updated
  Revision History:  08/01/13 Grant Dvorak updated
  Revision History:  08/06/13 Grant Dvorak updated
  Revision History:  08/15/13 Grant Dvorak updated
  Revision History:  08/20/13 Grant Dvorak updated
  Revision History:  09/20/13 Grant Dvorak updated
  Revision History:  10/09/13 Grant Dvorak updated
...
-->
<!-- Grant Dvorak - 8/20/2013 - Added sdtc namespace -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc">
  ...
  <xsl:template match="n1:ClinicalDocument">
    <html>
      ...
      <body>
        ...
        <!-- Dave Blaeser - 4/26/2013 - Change to left justify -->
        <h1 class="h1">
          <xsl:value-of select="$title"/>
        </h1>

        <!-- START display top portion of clinical document -->
        <!-- Dave Blaeser - 4/26/2013 - Only call recordTarget (Patient) and visitInformation (Date/Location) for the header -->
        <xsl:call-template name="recordTarget"/>
        <xsl:call-template name="visitInformation"/>
        ...
        <!-- Grant Dvorak - 10/09/2013 - Added template to generate a Care Team section when one doesn't exist -->
        <xsl:if test="not(//n1:component/n1:section[n1:title = 'Care Team'])">
          <xsl:call-template name="make-careteam"/>
        </xsl:if>

        <!-- Dave Blaeser - 4/26/2013 - Move display of Document ID and Date to the end of the document -->
  ...
  <xsl:template name="make-tableofcontents">
    ...
    <ul>
      ...
      <!-- Grant Dvorak - 10/09/2013 - Added code to generate a Care Team table of contents entry when there is no Care Team section -->
      <xsl:if test="not(n1:component/n1:structuredBody/n1:component/n1:section[n1:title = 'Care Team'])">
        <li>
          <a href="#{generate-id(n1:documentationOf/n1:serviceEvent/n1:performer)}">Care Team</a>
        </li>
      </xsl:if>
    </ul>
  </xsl:template>

  <!-- Dave Blaeser - 4/26/2013 - Display as text instead of table -->
  <!-- documentGeneral -->
  ...
  <!-- visitInformation -->
  <!-- Dave Blaeser - 4/26/2013 - New template to display Visit Date and Location -->
  <!--    Borrowed from componentOf and documentationOf with extensive modifications -->
  <xsl:template name="visitInformation">

    <!-- Visit Date -->
    <br/>
    <span class="td_label">
      <xsl:text>Visit Date: </xsl:text>
    </span>

    <xsl:choose>

      <!-- First try to get the Visit Date from documentationOf/ServiceEvent -->
      <xsl:when test="n1:documentationOf/n1:serviceEvent/n1:effectiveTime/n1:low/@value">
        <xsl:for-each select="n1:documentationOf/n1:serviceEvent">
          <xsl:if test="n1:effectiveTime/n1:low">
            ...
            <xsl:if test="n1:effectiveTime/n1:high">
              <xsl:if test="n1:effectiveTime/n1:low/@value != n1:effectiveTime/n1:high/@value">
      ...
      <!-- Otherwise, check in componentOf/encompassingEncounter -->
      <xsl:otherwise>
    ...
    <br/>
    <span class="td_label">
      <xsl:text>Visit Location: </xsl:text>
    </span>
    <xsl:for-each select="n1:componentOf/n1:encompassingEncounter">
      <xsl:choose>
        <xsl:when test="n1:location/n1:healthCareFacility/n1:location/n1:name">
          ...
          <xsl:call-template name="show-address-singleLine">
  ...
  <!-- recordTarget -->
  <!-- Dave Blaeser - 4/26/2013 - Change display of patient demographics -->
          ...
          <xsl:if test="not(n1:id/@nullFlavor)">
            <tr>
              ...
              <td class="td_patient">
                ...
                <!-- Grant Dvorak - 9/20/2013 - Added n1 raceCode back in. Now we display n1 *and* sdtc raceCode. -->
                <!-- Grant Dvorak - 10/09/2013 - Added [not(@nullFlavor)] checks to both raceCode elements. -->
                <xsl:if test="n1:patient/n1:raceCode">
                  <xsl:for-each select="n1:patient/n1:raceCode[not(@nullFlavor)]">
                    <!-- Grant Dvorak - 8/01/2013 - Added commas between race values -->
                    <xsl:if test="position() &gt; 1">, </xsl:if>
                    <xsl:call-template name="show-race-ethnicity"/>
                  </xsl:for-each>
                </xsl:if>
                <!-- Grant Dvorak - 8/20/2013 - Changed raceCode from n1 to sdtc namespace -->
                <xsl:if test="n1:patient/sdtc:raceCode[not(@nullFlavor)]">
                ...
                <xsl:if test="n1:patient/n1:languageCommunication/n1:languageCode">
                  <xsl:for-each select="n1:patient/n1:languageCommunication/n1:languageCode">
                    <!-- Grant Dvorak - 8/06/2013 - Added commas between language values -->
                    <xsl:if test="position() &gt; 1">, </xsl:if>
              ...
              <td class="td_patient">
                ...
                <xsl:for-each select="n1:id">
                  <xsl:choose>
                    <xsl:when test="contains(@root,'2.2')">
                      <!--test="position()=2">-->
                      <xsl:call-template name="show-patientid"/>
                    </xsl:when>
                  </xsl:choose>
                </xsl:for-each>
 ...
 @mediaType='image/jpeg']">
  ...
  <!-- Dave Blaeser - 4/26/2013 - New template to show the Patient ID -->
  <!--  show-patientid -->
  <xsl:template name="show-patientid">
    <xsl:param name="id"/>
    <xsl:choose>
      <xsl:when test="not($id)">
        <xsl:if test="not(@nullFlavor)">
  ...
  <xsl:template name="show-gender">
    <xsl:choose>
      ...
      <!-- Grant Dvorak - 8/15/2013 - Added UN gender code -->
  ...
  <!-- Dave Blaeser - 4/26/2013 - New template for preferred-language -->
  <!-- Grant Dvorak - 9/20/2013 - Converted ISO 639-1 codes to ISO 639-2 -->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:choose>
          ...
          <xsl:when test="@code = 'eng-US' ">
            <xsl:text>English, US</xsl:text>
          </xsl:when>
          <xsl:when test="@code = 'spa' ">
            <xsl:text>Spanish</xsl:text>
          </xsl:when>
          <xsl:when test="@code = 'spa-US' ">
            <xsl:text>Spanish, US</xsl:text>
          </xsl:when>
          ...
          <xsl:when test="@code = 'iro' ">
            <xsl:text>Iroquoian</xsl:text>
          </xsl:when>
          <xsl:when test="@code = 'sio' ">
            <xsl:text>Siouan</xsl:text>
          </xsl:when>
          <xsl:when test="@code = 'ath' ">
            <xsl:text>Athapascan</xsl:text>
          </xsl:when>
  ...
  <!-- Dave Blaeser - 4/26/2013 - Create new template to show address horizontally on a single line-->
  <!-- show-address-singleLine -->
  <xsl:template name="show-address-singleLine">
    <xsl:param name="address"/>
    <xsl:choose>
      <xsl:when test="$address">
        <xsl:for-each select="$address/n1:streetAddressLine">
...
<!-- Dave Blaeser - 4/26/2013 - Change background color -->
.narr_tr {
...
<!-- Dave Blaeser - 4/26/2013 - Remove color -->
.td_label{
  font-weight: bold;
  <!--color: #E8E8E8;-->
}

<!-- Dave Blaeser - 4/26/2013 - Create new class for patient demographics display -->
.td_patient {
  background-color: #FFFFFF;
  vertical-align: top;
  width: 50%;
  ...
  <!-- Grant Dvorak - 10/09/2013 - Added templates to generate a Care Team section when one doesn't exist -->
  <xsl:template name="make-careteam">
    <h3>
      <a name="{generate-id(n1:documentationOf/n1:serviceEvent/n1:performer)}" href="#toc">
        Care Team
      </a>
    </h3>
    <div>
      <!-- style="margin-left : 2em;"> -->
      <xsl:for-each select="/n1:ClinicalDocument/n1:documentationOf/n1:serviceEvent/n1:performer/n1:assignedEntity[not(n1:assignedPerson/n1:name/@nullFlavor)]">
        <xsl:call-template name="careteamprovider"/>
        <br/>
      </xsl:for-each>
      <xsl:for-each select="/n1:ClinicalDocument/n1:componentOf/n1:encompassingEncounter/n1:encounterParticipant/n1:assignedEntity[not(n1:assignedPerson/n1:name/@nullFlavor)]">
        <xsl:call-template name="careteamprovider"/>
        <br/>
      </xsl:for-each>
    </div>
    <xsl:if test="count(/n1:ClinicalDocument/n1:documentationOf/n1:serviceEvent/n1:performer/n1:assignedEntity/n1:assignedPerson/n1:name[not(@nullFlavor)]) = 0">
      &#160;&#160;&#160;No Care Team Information for the extraction criteria
      <br/>
    </xsl:if>
  </xsl:template>
  <xsl:template name="careteamprovider">
    &#160;&#160;
    <!-- Provider name -->
    <xsl:if test="not(n1:assignedPerson/n1:name/@nullFlavor)">
      <xsl:for-each select="n1:assignedPerson/n1:name/n1:prefix">
        <xsl:value-of select="."/>&#160;
      </xsl:for-each>
      <xsl:for-each select="n1:assignedPerson/n1:name/n1:given">
        <xsl:value-of select="."/>&#160;
      </xsl:for-each>
      <xsl:for-each select="n1:assignedPerson/n1:name/n1:family">
        <xsl:value-of select="."/>
        <xsl:if test="position() != last()">&#160;</xsl:if>
      </xsl:for-each>
      <xsl:if test="n1:assignedPerson/n1:name/n1:suffix">,&#160;</xsl:if>
      <xsl:for-each select="n1:assignedPerson/n1:name/n1:suffix">
        <xsl:value-of select="."/>
        <xsl:if test="position() != last()">&#160;</xsl:if>
      </xsl:for-each>
    </xsl:if>
    ...
    <xsl:if test="not(n1:addr/@nullFlavor)">
      <xsl:if test="n1:addr">;&#160;</xsl:if>
      <xsl:for-each select="n1:addr[1]/n1:streetAddressLine">
        <xsl:value-of select="."/>,&#160;
      </xsl:for-each>
      <xsl:if test="not(n1:addr[1]/n1:city[1]/@nullFlavor)">
        <xsl:value-of select="n1:addr[1]/n1:city[1]"/>,&#160;
      </xsl:if>
      <xsl:if test="not(n1:addr[1]/n1:state[1]/@nullFlavor)">
        <xsl:value-of select="n1:addr[1]/n1:state[1]"/>&#160;
      </xsl:if>
      <xsl:value-of select="n1:addr[1]/n1:postalCode[1]"/>
    </xsl:if>
    <!-- Provider phone -->
    <xsl:if test="not(n1:telecom/@nullFlavor)">
      <xsl:if test="n1:telecom">;&#160;</xsl:if>
      <xsl:call-template name="format-telecom">
        <xsl:with-param name="telecom" select="n1:telecom[1]/@value"/>
      </xsl:call-template>
    </xsl:if>
  </xsl:template>
  <xsl:template name="format-telecom">
    <xsl:param name="telecom"/>
    <xsl:if test="starts-with($telecom,'tel:')">
      <xsl:value-of select="substring-after($telecom,'tel:')"/>
    </xsl:if>
  </xsl:template>
</xsl:stylesheet>