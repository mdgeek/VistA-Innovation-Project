 ...
 */
package org.carewebframework.vista.plugin.familyhistory.service;
...
public class FamilyHistoryService extends AbstractBrokerQueryService<FamilyMember> {
    ...
    protected List<FamilyMember> processData(IQueryContext context, String data) {
        ...
        for (String line : list) {
            ...
            if (!pcs[12].isEmpty()) {
                ...
                condition.setNote(note.length > 1 ? note[1].trim() : null);
    ...
    protected void createArgumentList(List<Object> args, IQueryContext context) {
        args.add(((Patient) context.getParam("patient")).getIdElement().getIdPart());
    }
    ...
    private void doAddEdit(FamilyMember member, Condition condition) {
        ...
        String result = service.callRPC("BGOFHX SET", PatientContext.getActivePatient().getIdElement().getIdPart(), params);
