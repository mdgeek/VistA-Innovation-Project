...
public class ImmunSelectorController extends BgoBaseController<String> {
	...
	public static String execute(String searchVac, boolean forceShow) {
		...
		ImmunSelectorController controller = (ImmunSelectorController) FrameworkController
				.getController(dlg);
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		RowComparator.autowireColumnComparators(lbVaccines.getListhead()
				.getChildren());
		loadForm();
	...
	private void loadForm() throws Exception {
		...
		if (colSort == null) {
			colSort = (Listheader) lbVaccines.getListhead().getChildren()
					.get(0);
		}
	...
	/**
	 * Returns the currently selected vaccine.
	 * 
	 * @Return the currently selected vaccine.
	 */
	private String getSelected() {
		return model.getSelection().toString();
	}
	...
	private String getRPCParam() {
		...
		result = "9999999.14^"
				+ StrUtil.xlate(txtSearch.getValue().toUpperCase(), U, "")
				+ "^^^^^" + sScreen + "^1^.01;1.14;.07";
		return result;
	}
