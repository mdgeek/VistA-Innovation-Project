...
public class PatientCaseDataController extends BgoBaseController<Object> {
    ...
    private final List<String> forecast = new ArrayList<String>();
    
    private final List<String> mother = new ArrayList<String>();
    
    private final List<String> inactReasons = new ArrayList<String>();
    
    private final List<String> caseData = new ArrayList<String>();
    ...
    private void getCaseData() {
        getBroker().callRPCList("BGOVIMM GETCASE", caseData, PatientContext.getActivePatient().getId().getIdPart());
    ...
    public void onClick$btnManager() {
        String val = LookupController.execute(Table.rtProvider, txtManager.getValue(), false, "I $D(~BIMGR(Y)),$$ACTPRV(Y)");
        if (val != null) {
    ...
    private Object toDAO() {
        ...
        appendData(sb,
            cboInactiveReason.getSelectedIndex() > -1 ? StringUtils.left(cboInactiveReason.getText().toLowerCase(), 1) : "");
