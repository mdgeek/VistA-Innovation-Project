...
public class AddImmunController extends BgoBaseController<Object> {
	...
	private String TC_NO_REF_REASON = "Refusal Reason not Selected";

	private List<String> refusalReasons = new ArrayList<String>();
	
	private List<String> lotNumbers = new ArrayList<String>();
	
	private List<String> vacSites = new ArrayList<String>();
	
	private List<String> vacElig = new ArrayList<String>();
	
	private List<String> vacReactions = new ArrayList<String>();
	
	private List<String> vacOverrides = new ArrayList<String>();
	
	private List<String> vacDefaults = new ArrayList<String>();
	...
	private final static Set<EncounterFlag> EF = EncounterFlag.flags(
			EncounterFlag.NOT_LOCKED, EncounterFlag.VALIDATE_ONLY);

	private final static Set<EncounterFlag> EF1 = EncounterFlag.flags(
			EncounterFlag.NOT_LOCKED, EncounterFlag.VALIDATE_ONLY,
			EncounterFlag.FORCE);

	private static int opFacility = 0;

	private static int opOther = 1;
	...
	public static void execute(ImmunItem item) {
		...
		AddImmunController controller = (AddImmunController) FrameworkController
				.getController(dlg);
		return;
	}
	...
	private void loadForm() {
		...
		if (immunItem != null) {
			...
			case HISTORICAL:
					...
					radOther.setSelected(txtLocation.getAttribute("ID") != null ? txtLocation.getAttribute("ID").toString().isEmpty() : false);
			...
			default:
					...
					loadVaccination();
					String locid = immunItem.getLocationID();
					txtLocation.setValue(immunItem.isImmunization() ? immunItem.getLocationName() : "");
					txtLocation.setAttribute("ID", immunItem.isImmunization() ? immunItem.getLocationID() : "");
					selLoc = immunItem.getLocationID() + U + U + immunItem.getLocationName();
					radOther.setSelected(txtLocation.getAttribute("ID") != null ? txtLocation.getAttribute("ID").toString().isEmpty() : false);
	...
	private void setEventType(EventType evType) {
		...
		fraReaction.setVisible(eventType == evType.CURRENT && immunItem != null ? true : false);
	...
	/**
	 *
	 *   Retrieves information specific to patient and selected vaccination.
	 *   First record:
	...
	private void loadVaccination() {
		String s = "";
		vacDefaults.clear();
		getBroker().callRPCList("BGOVIMM LOADIMM", vacDefaults, VistAUtil.parseIEN(PatientContext.getActivePatient()) + U + getVaccineID());
		...
		if (immunItem == null && !getVacContraindications().isEmpty()) {
			if (!PromptDialog.confirm("The patient has the following contraindications" +
					 " for " + txtVaccine.getText() + " of: " + StrUtil.CRLF +
					getVacContraindications() +  "Do you wish to continue?", "ALERT: Contraindications Exist")) {
				onClick$btnCancel();
			}
		}
	...
	public void onClick$btnLocation() {
		String val = LookupController.execute(Table.rtLocation,
			...
			if (i > 0) {
				...
				if (i == 10) {
					if (PromptDialog.confirm("Do you wish to save the reaction of '" + cboReaction.getText()
							+ "' as a contraindication for the patient?", "Save to Contraindications?")) {
	 ...
	 * @param Immunization
	 *            information
	 * @return V Immunization file IEN
	 * 
	 *         Add/Edit immunization INP = Visit IEN [1] ^ Historical [2] ^
	 *         Patient IEN [3] ^ Imm IEN [4]^ V File IEN [5] ^ Provider IEN [6]
	 *         ^ Location [7] ^ Other Location [8] ^ Imm Date [9] ^ Lot # [10] ^
	 *         Reaction [11] ^ VIS Date [12] ^ Dose Override [13] ^ Inj Site
	 *         [14] ^ Volume [15] ^ Counseled [16] ^ VFC Eligibility [17] ^
	 *         admin comments [18]
	 */
	private String toDAOI(ImmunItem record) {
		...
		if (radOther.isSelected()) {
			...
			appendData(sb, txtLocation.getAttribute("ID") != null ? txtLocation
					.getAttribute("ID").toString() : "");
			appendData(sb, txtLocation.getValue() != null ? txtLocation
					.getValue().toString() : "");
		}
		appendData(sb, eventType == EventType.CURRENT ? new FMDate(datGiven.getDate()).getFMDate() : new FMDate(datEventDate.getValue()).getFMDate());
		appendData(sb, eventType == EventType.CURRENT && cboLot.getSelectedIndex() > -1 ? cboLot.getSelectedItem().getValue() : "");
		appendData(sb, fraReaction.isVisible() && (cboReaction.getSelectedIndex() > -1) ? cboReaction.getSelectedItem().getValue() : "");
		appendData(sb, eventType == EventType.CURRENT ? new FMDate(datVIS.getValue()).getFMDate() : "");
		
		appendData(sb, fraReaction.isVisible() && (cboOverride.getSelectedIndex() > -1) ? cboOverride.getSelectedItem().getValue() : "");
        appendData(sb, eventType == EventType.CURRENT && (cboSite.getSelectedIndex() > -1) ? cboSite.getSelectedItem().getValue() : "");		
        ...
        appendData(sb, txtAdminNote.getValue().isEmpty() ? "@" : StrUtil.xlate(
					   txtAdminNote.getValue().toString(), StrUtil.CRLF, ""));
		return sb.toString();
	}
	 ...
	 * @param Patient
	 *            Refusal information
	 * @return Refusal file IEN
	 *
	 *         Add/edit a refusal INP = Refusal IEN [1] ^ Refusal Type [2] ^
	 *         Item IEN [3] ^ Patient IEN [4] ^ Refusal Date [5] ^ Comment [6] ^
	 *         Provider IEN [7] ^ Reason [8] Patch 5, changed mammogram code for
	 *         bilateral mammogram Patch 13,changes for SNOMED
	 */
	...
	private String toDAOR(ImmunItem record) {
		...
		String refIEN = record != null && record.getFileIEN() != null ? record
				.getFileIEN() : "";
		...
		appendData(sb, txtComment.getValue().isEmpty() ? "@" : StrUtil.xlate(
					   txtComment.getValue().toString(), StrUtil.CRLF, ""));
		appendData(sb, prv);
		appendData(sb, val);
		return sb.toString();
	}
	...
	private void appendData(StringBuilder sb, Object data) {
		sb.append(
				data == null ? ""
						: data instanceof IReferenceable ? ((IReferenceable) data)
								.getId().getIdPart() : data).append('^');
	}
	
    
	/**
	 *   @param Contraindication information
	 *   @return BI PATIENT CONTRAINDICATIONS File IEN
	 *   
	 *   Set contraindication record
	 *   */
	...
	private boolean validate() {
		...
		if (eventType == EventType.HISTORICAL){
			if ((datEventDate.getValue() == null) || txtLocation.getText().isEmpty()) {
				PromptDialog.showWarning("You must select a location and the date of the historical vaccination event.", BgoConstants.TC_NO_LOC_DATE);
				return false;
			}
		}
		...
		if (visitIEN.isEmpty() && eventType == EventType.CURRENT) {
			if (!EncounterUtil.ensureEncounter(EF1)) {
				PromptDialog.showWarning("Unable to establish a visit context to store this vaccination.", "Vaccination not Stored");
				return false;
			} else {
