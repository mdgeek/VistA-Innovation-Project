...
public class ImmunItem {
	...
	public String getVaccineName() {
		if (immunization != null) {
			return immunization.getImmunization().getProxiedObject()
					.getDisplay();
		} else {
			if (refusal != null) {
	...
	public boolean isType(char type) {
			switch (type) {
			case 'I': return immunization != null;
			case 'R': return refusal != null; 
			case 'C': return contraInd != null;
			case 'F': return forecast != null;
			}	
		return false;
	}
			...
			if (provider != null && !user.equals(provider)) {
				...
				if (!user.getLogicalId().equals(pcs[0])) {
					PromptDialog.showError("To delete the vaccination, you must either be the person that entered it or be "
						...
						+ "\nExaminer: " + provider.getName(), "Cannot Delete Vaccination");
					return false;
				}
			}
		}

		if (PromptDialog.confirm(BgoConstants.TX_CNFM_DEL + getVaccineName() + " " + (isType('R') ? "refusal?" : "vaccination?"),
				"Remove Patient Vaccination?")) {
				PCC.errorCheck(VistAUtil.getBrokerSession().callRPC(
				isType('R') ? "BGOREF DEL" : "BGOVIMM DEL",
				VistAUtil.concatParams(immunization != null ? immunization.getId().getIdPart() : refusal != null ? refusal.getId().getIdPart() : null)));
				return true;
		}
		return false;
	}
	...
	public boolean deleteContra() {
		if (isType('C')) {
			if (PromptDialog.confirm(BgoConstants.TX_CNFM_DEL + "contraindication of " + 
				...
				PCC.errorCheck(VistAUtil.getBrokerSession().callRPC("BGOVIMM DELCONT", VistAUtil.concatParams(contraInd.getId().getIdPart())));
				return true;
			}
		}
		return false;
