...
IMMUNIZATION MANAGEMENT 1.0
"BLD",8518,0)
IMMUNIZATION MANAGEMENT 1.0^^0^3151027^n
"BLD",8518,4,0)
^9.64PA^^
"BLD",8518,6.3)
11
...
BGOVIMM^^0^B189809046
"BLD",8518,"KRN",9.8,"NM",2,0)
BGOVIMM2^^0^B46146353
...
"BLD",8518,"KRN",8994,"NM","B","BGOVIMM GETSITES",17)
...
"BLD",8518,"KRN",8994,"NM","B","BGOVIMM RPMS",16)
...
0^16
"KRN",8994,2803,0)
BGOVIMM RPMS^RPMS^BGOVIMM^1
"KRN",8994,2804,-1)
0^17
"KRN",8994,2804,0)
BGOVIMM GETSITES^GETSITES^BGOVIMM2^4^^^^1
"MBREQ")
0
"ORD",16,8994)
...
0^1^B189809046
"RTN","BGOVIMM",1,0)
BGOVIMM ;IHS/BAO/TMD - IMMUNIZATION mgt   ;27-Oct-2015 19:13;DKM
"RTN","BGOVIMM",2,0)
 ;;1.1;BGO COMPONENTS;**1,3,4,5,6,9,10,11,12,13**;Mar 20, 2007;Build 11
"RTN","BGOVIMM",3,0)
...
"RTN","BGOVIMM",75,0)
 N XREF,FNUM,INJF,BIPDSSA,OR,LOT,LOTIEN,MANUF
"RTN","BGOVIMM",76,0)
 S RET=$$TMPGBL^BGOUTL
"RTN","BGOVIMM",77,0)
...
"RTN","BGOVIMM",82,0)
 S XREF=$$VFPTXREF^BGOUTL2,FNUM=$$FNUM,INJF=$S($$RPMS:.09,1:1303)
"RTN","BGOVIMM",83,0)
...
"RTN","BGOVIMM",137,0)
 E  D
"RTN","BGOVIMM",138,0)
 .N REC,LP,X,X13,Y,Z,FNUM
"RTN","BGOVIMM",139,0)
 .S FNUM=9000010.11,INJF=1303
"RTN","BGOVIMM",140,0)
 .S LP="" F  S LP=$O(^AUPNVIMM("C",DFN,LP)) Q:'LP  D
"RTN","BGOVIMM",141,0)
 ..S X=$G(^AUPNVIMM(LP,0)),X13=$G(^(13))
"RTN","BGOVIMM",142,0)
 ..Q:$P(X,U,2)'=DFN
"RTN","BGOVIMM",143,0)
...
"RTN","BGOVIMM",162,0)
 ..S REC=REC_U_$P(X,U,9)_"~"_$$GET1^DIQ(FNUM,LP,INJF)                  ; Inj Site
"RTN","BGOVIMM",163,0)
 ..S REC=REC_U_$P(X13,U,12)                                            ; Volume
"RTN","BGOVIMM",164,0)
...
"RTN","BGOVIMM",314,0)
 N EVNTDT,LOCIEN,OUTLOC,HIST,CAT,INJS,IMMNM,FNUM,VFNEW,ARG,INJF,VFC,VFCIEN
"RTN","BGOVIMM",315,0)
 S RET="",FNUM=$$FNUM,V="|"
"RTN","BGOVIMM",316,0)
 S INJF=$S($$RPMS:.09,1:1303)
"RTN","BGOVIMM",317,0)
 S VIEN=+INP
"RTN","BGOVIMM",318,0)
 S HIST=$P(INP,U,2)
...
"RTN","BGOVIMM",341,0)
 S INJS=$P(INP,U,14)                                        ; Immunization Site
"RTN","BGOVIMM",342,0)
 S INJS=$S($$RPMS:$$TOINTRNL^BGOUTL(FNUM,INJF,INJS),1:"-"_INJS)
"RTN","BGOVIMM",343,0)
...
0^2^B46146353
"RTN","BGOVIMM2",1,0)
BGOVIMM2 ;IHS/BAO/TMD  BGO - IMMUNIZATION mgt;27-Oct-2015 19:06;DKM
"RTN","BGOVIMM2",2,0)
 ;;1.1;BGO COMPONENTS;**1,3,4,6,10,11,12,13**;Mar 20, 2007;Build 11
"RTN","BGOVIMM2",3,0)
 ;---------------------------------------------
"RTN","BGOVIMM2",4,0)
 ; Get case data
...
"RTN","BGOVIMM2",211,0)
 Q
"RTN","BGOVIMM2",212,0)
GETSITES(RET) ; Return routes of administration
"RTN","BGOVIMM2",213,0)
 I $$RPMS^BGOVIMM D
"RTN","BGOVIMM2",214,0)
 .N ENT
"RTN","BGOVIMM2",215,0)
 .S RET=$$TMPGBL^BGOUTL
"RTN","BGOVIMM2",216,0)
 .D SETVALS^RGUTRPC(.ENT,9000010.11,.09)
"RTN","BGOVIMM2",217,0)
 .M @RET=ENT
"RTN","BGOVIMM2",218,0)
 E  D
"RTN","BGOVIMM2",219,0)
 .D FILENT^RGUTRPC(.RET,920.3)
"RTN","BGOVIMM2",220,0)
 Q
"VER")
8.0^22.0
