 ...
 */
package org.carewebframework.vista.plugin.patientgoals.service;
...
public class GoalService extends AbstractBrokerQueryService<Goal> {
    ...
    protected void createArgumentList(List<Object> args, IQueryContext context) {
        ...
        args.add(patient.getIdElement().getIdPart());
    ...
    public <T extends Enum<T>> T toEnum(String code, Class<T> clazz) {
        code = StrUtil.piece(code, ";");
        return code.isEmpty() ? null : Enum.<T> valueOf(clazz, code);
    }
    ...
    public void addGoal(Goal goal) {
        ...
        flush(sb, data);
        String result = service.callRPC("BEHOPGAP ADDGOAL", goal.getPatient().getIdElement().getIdPart(), data);
    ...
    private float nextGoalNumber(Patient patient) {
        String result = service.callRPC("BEHOPGAP NEXTGN", patient.getIdElement().getIdPart(), getLocationIEN());
